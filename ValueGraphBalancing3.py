...
def init_matrix(negative_interaction_matrix_dict, positive_interaction_matrix_dict):
    ...
    for value1, value1_data in negative_interaction_matrix_dict.items():
        index1 = value_names.index(
            value1
        )  # do not use enumerate() here for case the value_names are in a different order
        for value2, interaction in value1_data.items():
            index2 = value_names.index(
                value2
            )  # cannot use enumerate() here since not all keys are present
    ...
    )  # check that the matrix was populated correctly - the matrix has to be symmetric
...
def plot_agent_history(
    subplots, plots_column, agent_name, values_history, utilities_history
):
...
def plot_history(
    values_history_dict, utilities_history_dict, utility_function_mode, rebalancing_mode
):
    fig, subplots = plt.subplots(2, 4)

    fig.suptitle(
        f"Value graph balancing - utility function: {utility_function_mode} - rebalancing: {rebalancing_mode}"
    )
...
def compute_utilities(
    prev_actual_values, updated_actual_values, prev_utilities, utility_function_mode
):
    ...
    if utility_function_mode == "linear":
        ...
        utility_function_mode == "prospect_theory"
    ):  # sigmoid is applied to value CHANGES not to RESULTING values. ALSO: negative side is amplified.
        # NB! current logic amplifies LOSS, irrespective whether the resulting value is positive of negative.
        change_utilities = (
            custom_sigmoid(positive_value_changes)
            + custom_sigmoid(negative_value_changes) * 2
        )  # TODO: config parameter
        # utilities = prev_utilities + change_utilities
        utilities = (
            0.5 * prev_utilities + change_utilities
        )  # TODO: parameter for past utilities discounting

    elif (
        utility_function_mode == "concave"
    ):  # positive side is logarithmic similarly to sigmoid, but negative side is treated exponentially
        ...
        utility_function_mode == "linear_homeostasis"
    ):  # too much of an actual value reduces the subjective value (utility)
        ...
        utility_function_mode == "squared_homeostasis"
    ):  # too much of an actual value reduces the subjective value (utility)
        diff_from_targets = np.abs(updated_actual_values - target_values)
        # diff_from_targets = np.power(diff_from_targets, 2)    # TODO: parameter
...
def main(utility_function_mode, rebalancing_mode):
    ...
    for agent_name in agent_names:
        ...
        if (
            utility_function_mode == "linear_homeostasis"
            or utility_function_mode == "squared_homeostasis"
        ):  # NB! in case of homeostatic utilities, the initial values cannot be too far off targets, else the system never recovers
            actual_values = (
                homeostatic_utility_scenario_actual_values.copy()
            )  # NB! copy since this matrix might be modified in place later
        else:
            actual_values = (
                initial_actual_values.copy()
            )  # NB! copy since this matrix might be modified in place later
        utilities = compute_utilities(
            prev_actual_values, actual_values, prev_utilities, utility_function_mode
        )
    ...
    for step in range(0, experiment_length):
        ...
        for agent_index, agent_name in enumerate(agent_names):
            ...
            if not restrict_negative_interactions:
                value_changes1 = (
                    np.matmul(other_utilities, interaction_matrix) * change_rate
                )
            else:
                positive_interaction_value_changes = (
                    np.matmul(other_utilities, positive_interaction_matrix)
                    * change_rate
                )
                negative_interaction_value_changes = (
                    np.matmul(
                        np.maximum(other_utilities, 0), negative_interaction_matrix
                    )
                    * change_rate
                )  # np.maximum: in case of negative interactions, ignore negative actual values
                value_changes1 = (
                    positive_interaction_value_changes
                    + negative_interaction_value_changes
                )
            ...
            if not restrict_negative_interactions:
                value_changes2 = (
                    np.matmul(self_utilities, interaction_matrix) * change_rate
                )
            else:
                positive_interaction_value_changes = (
                    np.matmul(self_utilities, positive_interaction_matrix) * change_rate
                )
                negative_interaction_value_changes = (
                    np.matmul(
                        np.maximum(self_utilities, 0), negative_interaction_matrix
                    )
                    * change_rate
                )  # np.maximum: in case of negative interactions, ignore negative actual values
                value_changes2 = (
            ...
            self_updated_actual_values = (
                self_actual_values + value_changes1 + value_changes2
            )
        ...
        if rebalancing_mode == "none":
            ...
            if deviation < 0:
                balance_step = min(
                    max_rebalancing_step_size, -deviation
                )  # min(): if deviation magnitude is smaller than max_rebalancing_step_size then step by deviation magnitude only
            else:
                balance_step = -min(
                    max_rebalancing_step_size, deviation
                )  # min(): if deviation magnitude is smaller than max_rebalancing_step_size then step by deviation magnitude only
            ...
            rebalancing_mode == "homeostatic_boosting"
        ):  # TODO: implement also naive boost mode which chooses a value with lowest level regardless of the target value
            ...
            rebalancing_mode == "homeostatic_throttling"
        ):  # TODO: implement also naive throttling mode which chooses a value with highest level regardless of the target value
        ...
        utilities = compute_utilities(
            actual_values, rebalanced_actual_values, utilities, utility_function_mode
        )
